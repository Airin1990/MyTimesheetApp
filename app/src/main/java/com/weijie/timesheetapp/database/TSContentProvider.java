...
public class TSContentProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues contentValues) {
        ...
        switch (match) {
            case RECORDS:
                return insertTimesheet(uri, contentValues);
            default:
                throw new IllegalArgumentException("Insertion is not supported for uri " + uri);
        }
    }

    private Uri insertTimesheet(Uri uri, ContentValues values) {
        ...
        //TODO auto update created and updated

        //Following are performing insertion
        SQLiteDatabase db = tsdbHelper.getWritableDatabase();
        // TODO need to insert with valid RID generated at the backend, id should be same as RID
        long id = db.insert(TSContract.RecordEntry.TABLE_NAME, null, values);
        if (id == -1) {
            Log.e(LOG_TAG, "Fail to insert in sqlite");
            return null;
        }
